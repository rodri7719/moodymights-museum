...
module.exports=async function handler(req,res){res.setHeader('Cache-Control','no-store');const send=(code,data)=>res.status(code).json(JSON.parse(JSON.stringify(data,(_,v)=>typeof v==='bigint'?v.toString():v)));try{
 ...
 const ip=crypto.createHash('sha256').update(String(req.headers['x-real-ip']||req.socket?.remoteAddress||'unknown')).digest('hex');await limited(ip,'all',300,60);if(action==='result')await limited(ip,'verification-ip',60,3600);const body=typeof req.body==='string'?JSON.parse(req.body):req.body||{};if(Buffer.byteLength(JSON.stringify(body))>4000000)return send(413,{error:'Replay too large'});
 const rpc=createPublicClient({chain:abstract,transport:http(c.rpc)}),read=(functionName,args=[])=>rpc.readContract({address:c.contract,abi,functionName,args});
