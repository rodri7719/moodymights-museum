...
interface ILegacyVendettaV3 {function paused() external view returns(bool);function points(address) external view returns(uint256);function ownsCharacter(address,uint8) external view returns(bool);function owner() external view returns(address);function treasury() external view returns(address);function legacy() external view returns(address);function claimedRuns(bytes32) external view returns(bool);function priceSigner() external view returns(address);function rewardSigner() external view returns(address);}
/// @notice V3 preserves V2/V1 ownership and balances; Luca is paid, Spartano costs points. Players pay their own gas.
contract VendettaCourtV3 is Ownable2Step,Pausable,ReentrancyGuard,EIP712 {
 ILegacyVendettaV3 public immutable legacy;
 ILegacyVendettaV3 public immutable original;
 ...
 constructor(address admin,address payable recipient,address previous)Ownable(admin)EIP712("VendettaCourt","3"){
  if(recipient==address(0)||previous.code.length==0)revert Invalid();legacy=ILegacyVendettaV3(previous);
  if(legacy.owner()!=admin||legacy.treasury()!=recipient)revert Invalid();treasury=recipient;testers[admin]=true;
  original=ILegacyVendettaV3(legacy.legacy());
  if(address(original).code.length==0||original.owner()!=admin||original.treasury()!=recipient)revert Invalid();
  priceSigner=legacy.priceSigner();rewardSigner=legacy.rewardSigner();priceEpoch=1;signerEpoch=1;
 }
 ...
 function economyVersion()external pure returns(uint256){return 3;}
 function claimedRuns(bytes32 id)public view returns(bool){return redeemed[id]||legacy.claimedRuns(id)||original.claimedRuns(id);}
 function pointPrice(uint8 id)public pure returns(uint256){if(id==1)return 5000;if(id==2)return 10000;if(id==4)return 15000;revert Invalid();}
 function _migrate(address player)private{if(!legacy.paused()||!original.paused())revert LegacyNotFrozen();if(migrated[player])return;uint8 bits=1;for(uint8 i=1;i<=4;i++)if(legacy.ownsCharacter(player,i))bits|=uint8(1)<<i;balances[player]=legacy.points(player);rosters[player]=bits;migrated[player]=true;emit Migrated(player,balances[player],bits);}
 ...
 function buyLucaNetz(Price calldata price,bytes calldata signature)external payable whenNotPaused allowed nonReentrant{
  _migrate(msg.sender);if(ownsCharacter(msg.sender,3))revert AlreadyOwned();
  ...
  rosters[msg.sender]|=8;collectedFees+=msg.value;emit CharacterUnlocked(msg.sender,3,0,msg.value);
 }
 function claimReward(Reward calldata r,bytes calldata signature)external payable whenNotPaused allowed nonReentrant{
  ...
  if(!SignatureChecker.isValidSignatureNow(rewardSigner,digest,signature))revert BadSignature();redeemed[r.runId]=true;balances[msg.sender]+=r.points;collectedFees+=msg.value;emit RewardClaimed(msg.sender,r.runId,r.points,r.feeWei);
 }
