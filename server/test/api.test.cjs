...
test('API requires wallet auth; ignores claimed wins; consumes matches; signs only verified closed runs',async()=>{
 ...
 // Issue all chapters without replays; only ordered server results create points.
 r=(await request('new-run',{character:'penguin'},token)).body.run;
 const issued=[];const before=verifierCalls;
 for(let i=0;i<5;i++){const m=await request('match',{runId:r.id,deferred:true,attempt:i,stage:i},token);assert.equal(m.status,200);issued.push(m.body.ticket);assert.equal(m.body.run.earned,0)}
 assert.equal(verifierCalls,before);
 assert.equal((await request('claim',{runId:r.id},token)).status,400);
 assert.equal((await request('result',{runId:r.id,matchId:issued[1].id,proof:{frames:[[0,0,[]]]}},token)).status,400);
 assert.equal((await request('match',{runId:r.id,deferred:true,attempt:7,stage:4},token)).status,400);
 for(let i=0;i<5;i++){const result=await request('result',{runId:r.id,matchId:issued[i].id,proof:{frames:[[0,0,[]]],win:false}},token);assert.equal(result.status,200);r=result.body.run}
 assert.equal(r.earned,1000);assert.equal(r.closed,true);
 assert.equal((await request('claim',{runId:r.id},token)).body.reward.points,'1000');
 const checked=verifierCalls;assert.equal((await request('result',{runId:r.id,matchId:issued[0].id,proof:{frames:[[0,0,[]]]}},token)).status,200);assert.equal(verifierCalls,checked);
 // A provisional victory cannot unlock a stage when its actual replay loses.
 verifierWins=false;r=(await request('new-run',{character:'penguin'},token)).body.run;
 const a=(await request('match',{runId:r.id,deferred:true,attempt:0,stage:0},token)).body.ticket;
 const b=(await request('match',{runId:r.id,deferred:true,attempt:1,stage:1},token)).body.ticket;
 assert.equal((await request('result',{runId:r.id,matchId:a.id,proof:{frames:[[0,0,[]]],win:true}},token)).body.run.earned,0);
 assert.equal((await request('result',{runId:r.id,matchId:b.id,proof:{frames:[[0,0,[]]],win:true}},token)).status,400);

 process.env.VENDETTA_CONTRACT_V3='0x3333333333333333333333333333333333333333';
 const quote=await request('price',{},token);assert.equal(quote.status,200);
 const Price=[{name:'player',type:'address'},{name:'priceWei',type:'uint256'},{name:'deadline',type:'uint256'},{name:'epoch',type:'uint256'}];
 const domain={name:'VendettaCourt',version:'3',chainId:2741,verifyingContract:process.env.VENDETTA_CONTRACT_V3};
 const recovered=await viem.recoverTypedDataAddress({domain,types:{Price},primaryType:'Price',message:quote.body.price,signature:quote.body.signature});assert.equal(recovered.toLowerCase(),rewardSigner.address.toLowerCase());
 assert.notEqual((await viem.recoverTypedDataAddress({domain:{...domain,version:'2'},types:{Price},primaryType:'Price',message:quote.body.price,signature:quote.body.signature})).toLowerCase(),rewardSigner.address.toLowerCase());
 delete process.env.VENDETTA_CONTRACT_V3;
